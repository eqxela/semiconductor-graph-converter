...
1. **Adobe Illustrator 편집 호환성 극대화 (`svg.fonttype: "none"`)**
   * Matplotlib에서 SVG로 저장할 때 글자가 그림(path)으로 쪼개지지 않고 **실제 텍스트(Arial 폰트) 그대로 살아있게 저장**됩니다. 일러스트레이터에서 텍스트 더블클릭만으로 손쉽게 편집(폰트 변경, 텍스트 수정)이 가능합니다.
2. **의존성 패키지 자동 설치**
...
3. 화면의 **`찾아보기... (Browse...)`** 버튼을 클릭하여 엑셀 데이터가 모여 있는 대상 폴더를 선택합니다.
   * *Tip: 해당 폴더 안의 모든 하위 폴더까지 재귀적으로 스캔합니다.*
4. **`그래프 변환 시작 (Run Convert)`** 버튼을 누릅니다.
5. 변환 결과가 로그 창에 실시간 출력되며, 완료 팝업이 뜨면 작업이 끝난 것입니다.

### 방법 B. CLI 모드 (개발자/터미널용)

백그라운드에서 빠르게 명령어로 처리할 때 유용합니다.

* 변환할 데이터 폴더 경로를 인자(Argument)로 넘겨서 실행합니다:
