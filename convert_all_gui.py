...
def process_file(file_path, presentation_mode=False):
    ...
    plt.close(fig)
    print(f"성공: {os.path.basename(svg_path)} 및 {os.path.basename(pdf_path)}")

# =========================================================
# 5. 전체 파일 스캔 및 실행 제어 함수
# =========================================================
def run_batch_conversion(search_dir, callback=None, presentation_mode=False):
    set_paper_style(presentation_mode=presentation_mode)
...
def launch_gui():
    ...
    # 옵션 설정 프레임 (Presentation Preset Mode Checkbox)
    options_frame = tk.LabelFrame(main_frame, text=" 옵션 설정 (Options) ", font=("Arial", 10, "bold"), bg="#f5f6f8", fg="#2F4EA2", padx=10, pady=5)
    options_frame.pack(fill="x", pady=5)
    
    pres_var = tk.BooleanVar(value=False)
    pres_chk = tk.Checkbutton(options_frame, text="발표용 (Presentation) 프리셋 모드 적용 (Larger Font, Thicker Line)", variable=pres_var, font=("Arial", 10), bg="#f5f6f8", activebackground="#f5f6f8")
    pres_chk.pack(anchor="w", padx=5)
    ...
    def start_conversion():
        ...
        try:
            is_pres = pres_var.get()
            success, total = run_batch_conversion(target, callback=update_gui, presentation_mode=is_pres)
            if total == 0:
